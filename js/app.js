...
setupEls.zoomInBtn.addEventListener("click", () => updateZoom(currentZoom + 0.1));
setupEls.zoomOutBtn.addEventListener("click", () => updateZoom(currentZoom - 0.1));
setupEls.zoomResetBtn.addEventListener("click", () => updateZoom(1.0));
...
function updateFrameSummary() {
  ...
  if (sessionState.frameType === "2x6") {
    let lines = [`Quantity: ${qty} sheet(s)`, ""];
    for (let i = 1; i <= qty; i++) lines.push(`Sheet ${i} → 2 identical strips`);
    lines.push("", `Total physical strips: ${qty * 2}`);
    frameEls.summaryText.textContent = lines.join("\n");
  } else {
    let lines = [`Quantity: ${qty} sheet(s)`, ""];
